...
def synConnectMap(netModel, nsiNeuron, numNeurons,
    ...
    kSynX1 = sp.linalg.solve(nsi1, )
    kSynY1 = sp.linalg.solve(nsi2), #np.ones_like(deltaY))
...
def FKJSNS3D(delay, numJoints, numSensPerJoint, numSteps,
    ...
    # P1 bell neurons
    for index in range(numSensPerJoint):
        nameStrP1 = 'P1bell' + str(index)
        net.add_neuron(neuron_type=bellNeur, name=nameStrP1)
        net.add_input(nameStrP1)
        net.add_output(nameStrP1)
    # P2 bell neurons
    for index in range(numSensPerJoint):
        nameStrP2 = 'P2bell' + str(index)
        net.add_neuron(neuron_type=bellNeur, name=nameStrP2)
        net.add_input(nameStrP2)
        net.add_output(nameStrP2)
    '''
    ...
    nsiP1 = np.empty_like(nsi23X)  # theta1 and P1 nsi compartments for the 2nd layer
    nsiP2 = np.empty_like(nsi23X) # theta1 and P2 nsi compartments for the 2nd layer
    ...
    to_simulate, gX, gY, gZ, gX1, gY1 = synConnectMap(net, NSIneuron, numSensPerJoint,
                                                      nsiX=nsi23X, nsiY=nsi23Y, nsiZ=nsiZ, nsi1=nsiP1, nsi2=nsiP2,
