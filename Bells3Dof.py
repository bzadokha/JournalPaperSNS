...
'''
A version for tests with two seperate synaptic connections functions: 
1st for NSIs between joints 2&3
2nd for NSIs between joint 1 and P0&P1 outputs from the 1st layer
'''
...
def synTuningLayer1(netModel, nsiNeuron, numNeurons, nsiX, nsiY, nsiZ,
                    mapX, mapZ, gI, synI, dEex, dEin, bellShift, jointTheta, bellWidth):
    '''
    ...
    :param dEx: excitatory synapse potential
    :param dEe: inhibitory synapse potential
    ...
    '''
    solving a linear equation nsi*k=map -> k = nsi^(-1)*map
    '''
    for i in range(numJoints - 1):
        kSyn[i, :] = sp.linalg.solve(nsiX, mapX[i, :])

    kSynX = kSyn[0, :] # 1st surface that gives P0
    kSynY = kSyn[1, :] # 2nd surface that gives P1
    ...
    for j in range(numNeurons):
        for k in range(numNeurons):
            ...
            if gZ[j, k] > 0:
                ...
                1 + 1
            n += 1
    return to_simulate, gX, gY, gZ

def synTuningLayer2(netModel, nsiNeuron, numNeurons, nsi1,
                  gI, synI, dEex, dEin,
    ...
    :param nsiNeuron: non-spiking neuron object from the SNS-toolbox
    :param numNeurons: number of sensory neurons
    :param nsi1: matrix that represents neuronal activity for Px and Py (empty)
    :param gI: synaptic conductance = Gmax|k=1
    :param synI: synaptic connection
    ...
    th4 = bell_curve(3, theta=1, shift=1, width=bellWidth)
    ...
    gX = np.zeros([numNeurons])
    gY = np.zeros([numNeurons])
    ...
    for j in range(numNeurons):
        ...
        gX[j] = Gmax(kSynX[j], mag, dEX1[j])
        gY[j] = Gmax(kSynY[j], mag, dEX1[j])

        if gX[j] > 0:
            synOutput = NonSpikingSynapse(max_conductance=float(gX[j]), reversal_potential=float(dEX1[j]),
                                          e_lo=mag, e_hi=2 * mag)
        ...
        if gY[j] > 0:
            synOutput = NonSpikingSynapse(max_conductance=float(gY[j]), reversal_potential=float(dEY1[j]),
                                          e_lo=mag, e_hi=2 * mag)
...
def FKJSNS3D(delay, numJoints, numSensPerJoint, numSteps,
    ...
    '''
        normalize surfaces using range [0, 1], get max and min values of each surface
    '''
    for i in range(numSensPerJoint):
...
P1x = np.zeros([numSensPerJointInit, len(t)])
plt.figure()

for i in range(numSensPerJointInit):
    P1x[i, :] = PredictedOutOne[:, 5 * numSensPerJointInit + 2 + i]
    plt.plot(P1x[i, :], label=str(jointTheta[i]))
    # Px = PredictedOutOne[:, 5*numSensPerJointInit+2 + i]*XcA[i]+minC[0, i]
    # Py = PredictedOutOne[:, 6*numSensPerJointInit+2 + i]*YcA[i]+minC[1, i]
    # Px = PredictedOutOne[:, 5 * numSensPerJointInit + 2 + i] * jointTheta[i]#*XcA[i]+minC[0, i]
    # Py = PredictedOutOne[:, 6 * numSensPerJointInit + 2 + i] * jointTheta[i]#*YcA[i]+minC[1, i]
    Px = PredictedOutOne[:, 5 * numSensPerJointInit + 2 + i]
    Py = PredictedOutOne[:, 6 * numSensPerJointInit + 2 + i]
    PredX[i, :] = Px
    PredY[i, :] = Py
plt.legend()
plt.show()
...
# yMapRotated = np.zeros_like(kSynX)
# degRot = np.pi / 4
# rotMatZ = np.array([[np.cos(degRot), -np.sin(degRot), 0], [np.cos(degRot), np.sin(degRot), 0], [0, 0, 1]])
# for i in range(len(jointTheta)):
#     for j in range(len(jointTheta)):
#         xyz = np.matmul(rotMatZ, np.array([[jointTheta[i]], [jointTheta[j]], [kSynX[i, j]]]))
#         # xyz = np.matmul(rotMat3DZ, np.array([[0], [0], [kSynX[i, j]]]))
#         yMapRotated[i, j] = xyz[2, 0]
