...
def synConnectMap(netModel, nsiNeuron, numNeurons,
    ...
    #         th4 = bell_curve(mag, theta=1, shift=1, width=bellWidth)
    #         # th4 = 0.5
    #         # print(th1 + np.transpose(th4) - mag)
    #
    #         nsi1[k1, :] = ComboVec(theta2Bell=th1, theta3Bell=th4, mag=mag, comboAct=nsi1, gMax=gI, delE=dEex)
    #         nsi2[k1, :] = ComboVec(theta2Bell=th1, theta3Bell=th4, mag=mag, comboAct=nsi1, gMax=gI, delE=dEex)
    #         # plt.figure()
    #         # plt.plot(nsi1[k1, :])
    #         # plt.show()
    ...
    th4 = bell_curve(2, theta=1, shift=1, width=bellWidth)
    ...
    kSynY1 = sp.linalg.solve(nsi1, deltaY)
...
def FKJSNS3D(delay, numJoints, numSensPerJoint, numSteps,
    ...
    '''
    # initialize neurons (they are outputs for 1st layer and inputs for 2nd layer)
    out1Neurons = 2
    ...
    for i in range(numSensPerJoint):
        mapNormX[i, :], maxCoef[0, i], minCoef[0, i] = Normalize(ActualXmap[i, :, :])
        mapNormY[i, :], maxCoef[1, i], minCoef[1, i] = Normalize(ActualYmap[i, :, :])
    mapNormZ, _, _ = Normalize(ActualZmap)
    '''
    ...
    YcANorm = ((YcA - min(YcA)) / (max(YcA) - min(YcA)))
    # print(np.size(XcA))
    # print(np.size(YcA))
    ...
    to_simulate, gX, gY, gZ, gX1, gY1 = synConnectMap(net, NSIneuron, numSensPerJoint,
                  ...
                  bellShift=jointThetaMat, jointTheta=jointTheta, bellWidth=widthBell, deltaX = XcANorm, deltaY = YcANorm)
    ...
    if to_simulate:
        for i in range(len(t)):
            inputNet[i, :] = np.concatenate((input1[i, :], np.concatenate((input2[i, :], input3[i, :]), axis=None) ), axis=None)
...
print('smth')
PredictedOutOne, ActualTOne, kSynX, kSynY, kSynZ, modelFKJ, _, xMap, yMap, zMap, maxC, minC,gX1, gY1 = FKJSNS3D(delay=delayInit,numJoints=numJoints,
...
PredX = np.zeros([numSensPerJointInit, len(t)])
PredY = np.zeros([numSensPerJointInit, len(t)])

for i in range(numSensPerJointInit):
    Px = PredictedOutOne[:, 52 + i]
    Py = PredictedOutOne[:, 62 + i]
    PredX[i, :] = Px
    PredY[i, :] = Py

Px = np.sum(PredX, axis=0)
Py = np.sum(PredY, axis=0)
plt.figure()
plt.plot((Px - min(Px)) / (max(Px) - min(Px)), color='red')
# plt.plot(Px)
plt.plot((ActualTOne[:, 0] - min(ActualTOne[:, 0])) / (max(ActualTOne[:, 0]) - min(ActualTOne[:, 0])))
# plt.plot(ActualTOne[:, 0])
plt.show()
plt.figure()
plt.plot((Py - min(Py)) / (max(Py) - min(Py)), color='k')
...
# ax1.plot(t, ActualTOne[:, 0], dashes=[6, 2], label='Actual', color='k')
# ax1.set_xlim([50*dt, tMax])
# # plt.legend()
...
# ax2.plot(t, ActualTOne[:, 1], dashes=[6, 2], label='Actual', color='k')
# plt.xlabel('Time, ms')
# ax2.set_xlim([50*dt, tMax])
# # plt.legend()
# # plt.legend(loc='center left', bbox_to_anchor=(1.15, 0.5))
# ax2.set_ylabel('Y, mm')
...
# ax3.plot(t, ActualTOne[:, 2], dashes=[6, 2], label='Actual', color='k')
# plt.xlabel('Time, ms')
# ax3.set_xlim([50*dt, tMax])
# # plt.ylabel('Z, mm')
# ax3.set_ylabel('Z, mm')
# # plt.legend()
# # plt.legend(loc='center left', bbox_to_anchor=(1.15, 0.5))
# # plt.savefig('PredActJacBellSfN.svg', bbox_inches='tight', dpi=500)
# plt.show()
# #
# plt.figure()
# plt.plot(t, predOutXOne, label='Predicted', color='red')
# plt.plot(t, ActualTOne[:, 0], dashes=[6, 2], label='Actual', color='k')
# # plt.show()
# # ax1.set_xlim([50*dt, tMax])
#
# theta22, theta33 = np.meshgrid(jointTheta, jointTheta)
# diffX = np.zeros([len(jointTheta), len(jointTheta), len(jointTheta)])
# coeffX = np.zeros([len(jointTheta), len(jointTheta), len(jointTheta)])
# diffX[0, :, :] = np.ones([len(jointTheta), len(jointTheta)])
# coeffX[0, :, :] = np.ones([len(jointTheta), len(jointTheta)])
# modeCoeff = np.zeros(len(jointTheta))
#
# ax = plt.figure().add_subplot(projection='3d')
# # ax.set(xlim=(-1.7, 1.7), ylim=(-1.7, 1.7), zlim=(-10, 10),
# #            xlabel='${\Theta}_{2}$, rad', ylabel='${\Theta}_{3}$, rad', zlabel='X, mm')
# for i in range(len(jointTheta)):
#     # Plot the 3D surface
#     if i == 0:
#         # ax.plot_surface(theta22, theta33, xMap[i, :, :], edgecolor='tomato', lw=0.5, rstride=11, cstride=11,
#         #                 alpha=0.5)
#         # ax.set_title('Xcoord')
#         diffX[i, :, :] = yMap[i, :, :]
#         ax.plot_surface(theta22, theta33, yMap[i, :, :], edgecolor='tomato', lw=0.5, rstride=11, cstride=11,
#                         alpha=0.5, label=str(jointTheta[i]))
#     else:
#         diffX[i, :, :] = yMap[i, :, :]-yMap[i-1, :, :]
#
#         coeffX[i, :, :] = diffX[i, :, :]/diffX[i-1, :, :]
#         ax.plot_surface(theta22, theta33, yMap[i, :, :], edgecolor='tomato', lw=0.5, rstride=11, cstride=11,
#                         alpha=0.5, label=str(jointTheta[i]))
#         # plt.show()
#
#     # plt.savefig('mapXbellSfN.svg', dpi=500)
#
# # plt.legend()
# ax.set_title('Ymap')
# plt.legend(loc='center left', bbox_to_anchor=(1.15, 0.5))
...
#                         alpha=0.5, label=str(jointTheta[i]))
#     else:
#         diffX[i, :, :] = xMap[i, :, :]-xMap[i-1, :, :]
#         # coeffX[i, :, :] = diffX[i, :, :]/diffX[i-1, :, :]
#         ax.plot_surface(theta22, theta33, xMap[i, :, :], edgecolor='tomato', lw=0.5, rstride=11, cstride=11,
#                         alpha=0.5, label=str(i+1))
#         # plt.show()
#         # ax.set_title('jT1=' + str(jointTheta[i]))
#     # plt.savefig('mapXbellSfN.svg', dpi=500)
#
# # plt.legend()
# ax.set_title('Xmap')
...
# for i in range(numSensPerJointInit):
#     # print(30+i)
#     plt.plot(PredictedOutOne[:, i])
#     plt.title('P1_'+str(i))
# plt.show()
#
# plt.figure()
# plt.plot(PredictedOutOne[:, 30])
# plt.title('P1')
# plt.show()
# plt.figure()
# plt.plot(PredictedOutOne[:, 31])
# plt.title('P2')
# plt.show()
...
#
# plt.figure()
# plt.plot(jointTheta, maxC[1, :])
# plt.plot(jointTheta, minC[1, :])
# plt.title('Ycoord')
# plt.show()
#
# plt.figure()
# plt.plot((maxC[0, :]-minC[0, :]))
# plt.title('deltaX')
# plt.show()
#
# plt.figure()
# plt.plot((maxC[1, :]-minC[1, :]))
# plt.title('deltaY')
# plt.show()
plt.figure()
for i in range(numSensPerJointInit):
    Px = PredictedOutOne[:, 32 + i]
    plt.plot(Px)
    plt.xlim([0, 1000])
plt.show()
plt.figure()
for i in range(numSensPerJointInit):
    Px = PredictedOutOne[:, 42 + i]
    plt.plot(Px)
    plt.title('Y')
    plt.xlim([0, 1000])
plt.show()
